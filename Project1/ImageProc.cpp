...
void ScreenShot::GetDesktopResolution(int& horizontal, int& vertical)
{
	 horizontal = ::GetSystemMetrics(SM_CXVIRTUALSCREEN) - abs(::GetSystemMetrics(SM_XVIRTUALSCREEN));
	 vertical = ::GetSystemMetrics(SM_CYVIRTUALSCREEN) - abs(::GetSystemMetrics(SM_YVIRTUALSCREEN));
...
BOOL ScreenShot::SaveToFile(HBITMAP hBitmap, LPCTSTR lpszFileName)
{
	HDC hDC;
...
*******************************************************************/
BYTE* ScreenShot::ConvertRGBToBMPBuffer(BYTE* Buffer, int width, int height, long* newsize)
{
...
*****************************************************************/
BYTE* ScreenShot::ConvertBMPToRGBBuffer(BYTE* Buffer, int width, int height)
{
	// first make sure the parameters are valid
	if ((NULL == Buffer) || (width == 0) || (height == 0))
		return NULL;
...
************************************************/
bool ScreenShot::LoadBMPIntoDC(HDC hDC, LPCTSTR bmpfile)
{
...
***************************************************************/
bool ScreenShot::SaveBMP(BYTE* Buffer, int width, int height, long paddedsize, LPCTSTR bmpfile)
{
...
program!!!
*******************************************************************/
BYTE* ScreenShot::LoadBMP(int* width, int* height, long* size, LPCTSTR bmpfile)
{
...
void ScreenShot::TestBMPCopy(LPCTSTR input, LPCTSTR output)
{
	int x, y;
	long s;
...
void ScreenShot::TestBMPCopy2(LPCTSTR input, LPCTSTR output)
{
	int x, y;
	long s, s2;
...
vector<COORD> ret;
	...
	try
	{
		for (int ssY = 0; ssY < yy; ssY++)
		{
			for (int ssX = 0; ssX < xx; ssX++)
			{
				...
				for (int bmpY = 0; bmpY < y; bmpY++)
				{
					...
					for (int bmpX = 0; bmpX < x; bmpX++)
					{
						...
						if (test[(bmpY*x + bmpX) * 3 + bmpY*(w - x)] == this->bmpBuffer[((ssY - bmpY)*xx + (ssX + bmpX)) * 4]
							&& test[(bmpY*x + bmpX) * 3 + bmpY*(w - x) + 1] == this->bmpBuffer[((ssY - bmpY)*xx + (ssX + bmpX)) * 4 + 1]
							&& test[(bmpY*x + bmpX) * 3 + bmpY*(w - x) + 2] == this->bmpBuffer[((ssY - bmpY)*xx + (ssX + bmpX)) * 4 + 2]
							)
						{
...
RGBA ScreenShot::getPixel(int x, int y)
{
...
BYTE * ScreenShot::getBytes(int ulx, int uly, int brx, int bry)
{
	//w = x + (x % 4);
	int xx = abs(brx - ulx);
...
BYTE * ScreenShot::hbitmapToBytes(HBITMAP input)
{
	char* pImage = NULL;
	HANDLE hImage = input;
...
HBITMAP ScreenShot::getSubBMP(int ulx, int uly, int brx, int bry)
{
