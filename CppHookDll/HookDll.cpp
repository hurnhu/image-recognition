...
LRESULT CALLBACK LowMouseHookProc(int nCode, WPARAM wParam, LPARAM lParam)
{
	...
	//check that the message is from keyboard or is synthesized by SendInput API
	if ((mshook->flags & LLMHF_INJECTED))
		return ::CallNextHookEx(g_hHook, nCode, wParam, lParam);
	...
	if (wParam == WM_KEYDOWN)
	{
		...
		::PostMessage(g_hWnd, WM_KEYINPUT, MAKEWPARAM(mshook->pt.x, mshook->pt.y), mshook->mouseData);
	}

	return ::CallNextHookEx(g_hHook, nCode, wParam, lParam);
}

BOOL WINAPI SetLowMouseHook(BOOL bInstall, DWORD dwThreadId, HWND hWndCaller)
{
	BOOL bOk;
	g_hWnd = hWndCaller;
	...
	if (bInstall)
	{
		g_hHook = ::SetWindowsHookEx(WH_KEYBOARD_LL, LowMouseHookProc,
